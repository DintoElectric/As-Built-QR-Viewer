...
export default function DrawingViewer({ sheet, panel, selCircuit, linked, sheetId, onPickSheet, full, onToggleFull }) {
  ...
  const circuitCount = selCircuit
    ? distinctLabels(placements.filter((b) => (b.circuits || []).includes(selCircuit))).length
    : 0;
  ...
  sheet.jboxes.forEach((b, i) => {
    ...
    if (b.panel === panel) {
      ...
      if (!selCircuit || (b.circuits || []).includes(selCircuit)) {
        ...
        const pad = 1.0 / k;
        dimmed.push(
          <rect key={'d' + i} x={x - pad} y={y - pad} width={w + 2 * pad} height={bh + 2 * pad} rx={1.5}
            fill="none" stroke="rgba(145,132,217,.28)" strokeWidth={0.8 / k} />
        );
      }
    } else {
      const pad = 0.9 / k;
